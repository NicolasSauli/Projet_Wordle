...
import { Users, Trophy, Play, LogIn, UserPlus, Home, Award } from 'lucide-react';
...
const WordleApp = () => {
  ...
  const loadStats = async (email) => {
    try {
      const statsData = await window.storage.get(`stats:${email}`);
      if (statsData) {
  ...
  const saveStats = async (email, newStats) => {
    try {
      await window.storage.set(`stats:${email}`, JSON.stringify(newStats));
      setStats(newStats);
    } catch (error) {
  ...
  const registerUser = async (email, nom, prenom) => {
  ...
  const createLobby = async (nomLobby) => {
    ...
    await window.storage.set(`lobby:${code}`, JSON.stringify(newLobby), true);
    setLobby(newLobby);
    setPage('lobby');
    showMessage(`Lobby créé! Code: ${code}`);
  };

  const joinLobby = async (code) => {
    try {
      const lobbyData = await window.storage.get(`lobby:${code}`, true);
      if (!lobbyData) {
        showMessage('Lobby introuvable!');
        return;
      }
      
      const existingLobby = JSON.parse(lobbyData.value);
      ...
      if (!alreadyJoined) {
        ...
        await window.storage.set(`lobby:${code}`, JSON.stringify(existingLobby), true);
      }
      ...
      showMessage('Erreur lors de la connexion au lobby');
    }
  };
  ...
  const corrigerGuess = (motSecret, guess) => {
    ...
    // Deuxième passe: jaunes et grises
    for (let i = 0; i < guessUpper.length; i++) {
      if (!result[i]) {
  ...
  const submitGuess = async () => {
    ...
    if (gagne) {
      ...
      await saveStats(user.email, newStats);
      ...
      await window.storage.set(`lobby:${lobby.code}`, JSON.stringify(updatedLobby), true);
      setLobby(updatedLobby);
      
      setGameOver(true);
      showMessage(`🎉 Bravo! Trouvé en ${newTentatives.length} essais! +${score} points`);
      ...
      await saveStats(user.email, newStats);
      setGameOver(true);
      showMessage(`Perdu! Le mot était: ${partie.motSecret}`);
    }
  };
      ...
      <div className="max-w-4xl mx-auto">
        <div className="bg-white rounded-2xl shadow-2xl p-8 mb-6">
          ...
          <div className="grid grid-cols-3 gap-4 mb-6">
            ...
            <div className="bg-green-100 p-4 rounded-lg text-center">
              <Play className="mx-auto mb-2 text-green-600" size={32} />
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <button
            ...
            className="bg-white hover:bg-gray-50 rounded-2xl shadow-lg p-8 text-center transition"
          >
            <LogIn className="mx-auto mb-4 text-pink-600" size={48} />
  ...
  return (
    <div className="relative">
      {message && (
        <div className="fixed top-4 left-1/2 transform -translate-x-1/2 bg-black text-white px-6 py-3 rounded-lg shadow-lg z-50 animate-pulse">
          {message}
        </div>
      )}
